...
export function SimpleListForm({ content: initial }: { content: SimpleListContent }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <span className="block text-xs uppercase tracking-[0.08em] text-ink/50">Rules</span>
        <div className="mt-2 space-y-2">
          {content.rules.map((rule, i) => (
            <div key={i} className="flex gap-2">
              <input className={inputClass + " mt-0"} value={rule} onChange={(e) => updateRule(i, e.target.value)} />
              <button type="button" onClick={() => removeRule(i)} className="px-2 text-xs text-ink/50 hover:text-red-700">
                Remove
              </button>
            </div>
          ))}
        </div>
        <button type="button" onClick={addRule} className="mt-3 border border-ink/20 px-4 py-2 text-sm hover:border-ink">
          + Add rule
        </button>
      </div>
