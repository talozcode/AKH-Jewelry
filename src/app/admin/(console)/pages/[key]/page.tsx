...
export default async function EditPagePage({ params }: { params: Promise<{ key: string }> }) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-medium">{PAGE_LABELS[pageKey]}</h1>
      <div className="mt-8 max-w-2xl">
        ...
        {pageKey === "shipping-returns" ? <SectionsForm content={await getPage("shipping-returns")} /> : null}
        ...
        {pageKey === "terms" ? <SimpleProseForm pageKey="terms" content={await getPage("terms")} /> : null}
      </div>
    </div>
  );
}
